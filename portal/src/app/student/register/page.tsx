...
/**
 * Public self-service registration (Session 18) — student side, mirrors
 * teacher.<root>/register exactly (see that page's header comment for the
 * full rationale). The subdomain is the platform-role choice; both call
 * the same src/lib/registration.ts's registerUser().
 */
export default async function StudentRegisterPage({
  ...
  async function register(formData: FormData) {
    ...
    const outcome = await registerUser({ name, email, password, role: "STUDENT" });
    if (!outcome.ok) {
  ...
  return (
    <div className={styles.page}>
      <div className={styles.card}>
        ...
        <p className={styles.subtitle}>Start learning on Keen Africa</p>
