...
export async function startGroupConversationAction(formData: FormData) {
  const actor = await requireActor();
  ...
  let error: string | null = null;
  let conversationId: string | null = null;
  if (recipientIds.length < 2 || !body.trim()) {
    error = "missing_fields";
  } else {
    try {
      const attachment = await attachmentFromFormData(formData);
      const { conversation } = await startConversation({ type: "group", participantIds: recipientIds, body, attachment }, actor);
      conversationId = conversation.id;
    } catch (err) {
      error = toError(err);
    }
...
export async function broadcastToCohortAction(formData: FormData) {
  const actor = await requireActor();
  ...
  let error: string | null = null;
  let conversationId: string | null = null;
  if (!cohortId || !body.trim()) {
    error = "missing_fields";
  } else {
    try {
      const attachment = await attachmentFromFormData(formData);
      const { conversation } = await startConversation({ type: "cohort_broadcast", cohortId, body, attachment }, actor);
      conversationId = conversation.id;
    } catch (err) {
      error = toError(err);
    }
