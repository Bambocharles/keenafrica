...
/**
 * Session 39 (Keen Africans — Notifications). Mirrors
 * src/app/student/(protected)/notifications/page.tsx exactly — same
 * canonical src/lib/notifications.ts read path, same read/unread/mark-read
 * behavior every other portal already has (Session 10's "notification
 * links to related entity" / "users can mark notifications read"
 * acceptance criteria). This page didn't exist before this session, so the
 * keenafricans layout's NotificationBell (added by Session 35) linked to a
 * 404 — see this session's handoff for that gap.
 */
...
export default async function KeenAfricansNotificationsPage({
  searchParams,
}: {
  ...
  return (
    <div style={{ display: "grid", gap: "20px" }}>
      ...
      {notifications.length === 0 ? (
        <EmptyState title="No notifications yet" hint="You'll see updates about your articles here." />
      ) : (
