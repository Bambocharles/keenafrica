...
- **Production** — `keen-prod` k8s namespace, deployed by
  ...
  approval. Its Postgres (`postgres01`) is **not** a single-purpose
  instance — it hosts `keenafrica_portal_prod` alongside three sibling
  databases on the same server/role catalog (`keenafrica_portal`, the
  confirmed-dormant pre-cutover database; `postgres`; `testdb`, untraced
  and confirmed empty). None are written to by the portal, and none are
  covered by `backup-portal-db.yml`. See `docs/BACKUP_RESTORE.md`'s
  "Infrastructure this assumes" for the full per-database detail and live
  evidence — found by Session 30, documented by Session 45.
...
| `LINKEDIN_CLIENT_ID` / `LINKEDIN_CLIENT_SECRET` | yes, for the Keen Africans "Connect LinkedIn" verification flow | OAuth/OIDC client credentials for the LinkedIn provider (`src/lib/auth.ts`) — see `docs/KEEN_AFRICANS.md`'s "Verification" section. Register `https://keenafricans.<root-domain>/auth/callback/linkedin` as an authorized redirect URI in the LinkedIn Developer Portal, with the "Sign In with LinkedIn using OpenID Connect" product enabled (requests `openid profile email` — no other scope exists; see that doc section for why). Without these set, "Connect LinkedIn" fails at the provider level; every other Keen Africans capability (registration, articles, Google sign-in) is unaffected — this is additive, not required for the platform to run. Not yet set in production as of Session 40 (this session shipped the mechanism; provisioning the real LinkedIn app is a deploy-time follow-up, same shape Session 22 did for `GOOGLE_CLIENT_ID`). |
